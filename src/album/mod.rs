...
use crate::{
    ...
    lidarr::{album_title, album_type, artist_name, bool_field, duration_ms, id, track_album_id, track_title, Lidarr},
...
impl AlbumConfig {
    fn from_env(args: &AlbumArgs) -> Self {
        Self {
            ...
            concurrency: args.concurrency.unwrap_or_else(|| env_parse("ALBUM_CONCURRENCY", 16)).max(1),
...
pub async fn run(args: AlbumArgs) -> Result<()> {
    ...
    let lidarr = Lidarr::new(app.lidarr_base_url.clone(), app.lidarr_header_value.clone(), cfg.concurrency);
    ...
    let eligible = artists.into_iter()
    ...
    let mut summary = Summary { artists_scanned: eligible.len(), ..Summary::default() };
    for artist in &eligible {
        ...
        if !args.force && cache.get(&artist_id).map(|row| cache_hit(row, &fp, cfg.cache_days)).unwrap_or(false) {
    ...
    for result in reports {
        match result {
            Ok(report) => {
                ...
                summary.releases_checked += report.duplicate_releases.len() + report.review_releases.len() + report.keep_releases.len();
...
async fn ensure_profile_exists(lidarr: &Lidarr, profile_id: i64) -> Result<()> {
    ...
    if profiles.iter().any(|p| id(p) == Some(profile_id)) { Ok(()) } else { Err(anyhow!("metadata profile {profile_id} not found")) }
}

async fn process_artist(lidarr: &Lidarr, store: &Store, cfg: &AlbumConfig, artist: Value, fingerprint: String) -> Result<ArtistReport> {
    let artist_id = id(&artist).ok_or_else(|| anyhow!("artist without id"))?;
    let name = artist_name(&artist);
    let current_profile = artist.get("metadataProfileId").and_then(Value::as_i64)
        .or_else(|| artist.pointer("/metadataProfile/id").and_then(Value::as_i64));
    let mut report = ArtistReport { artist_id, artist_name: name, status: "success".into(), fingerprint: fingerprint.clone(), metadata_profile_action: None, duplicate_releases: vec![], review_releases: vec![], keep_releases: vec![], changed_releases: vec![], errors: vec![] };

    if current_profile != Some(cfg.target_metadata_profile_id) {
        report.metadata_profile_action = Some(if cfg.apply { "updated" } else { "would_update" }.into());
        if cfg.apply {
            let mut updated = artist.clone();
            updated["metadataProfileId"] = json!(cfg.target_metadata_profile_id);
            lidarr.put(&format!("/artist/{artist_id}"), &updated).await?;
            let _ = lidarr.refresh_artist(artist_id).await;
        }
        store.upsert_artist(&ArtistCache { artist_id, fingerprint, processed_at: now_iso(), status: "success".into() }).await?;
        return Ok(report);
    }

    let albums = lidarr.albums_for_artist(artist_id).await?;
    let tracks = lidarr.tracks_for_artist(artist_id).await?;
    let mut tracks_by_album: HashMap<i64, Vec<Value>> = HashMap::new();
    for track in tracks {
    ...
    let full_albums = albums.iter().filter(|a| album_type(a) == "album").cloned().collect::<Vec<_>>();
    let mut candidate_releases = albums.iter().filter(|a| matches!(album_type(a).as_str(), "single" | "ep")).cloned().collect::<Vec<_>>();
    if cfg.only_monitored_releases { candidate_releases.retain(|a| bool_field(a, "monitored") == Some(true)); }
    ...
    for release in candidate_releases {
        ...
        let release_tracks = tracks_by_album.get(&release_id).cloned().unwrap_or_default();
        let decision = classify_release(&release_tracks, &full_track_index, cfg.duration_tolerance_ms);
        let item = json!({ "releaseId": release_id, "releaseTitle": album_title(&release), "releaseType": album_type(&release), "decision": decision, "trackCount": release_tracks.len() });
        match decision { "DUPLICATE" => report.duplicate_releases.push(item.clone()), "REVIEW" => report.review_releases.push(item), _ => report.keep_releases.push(item) }
        if decision == "DUPLICATE" && cfg.apply && cfg.auto_unmonitor_duplicates && release_id > 0 {
            ...
            if let Err(err) = lidarr.put(&format!("/album/{release_id}"), &album).await { report.errors.push(err.to_string()); } else { report.changed_releases.push(json!({ "releaseId": release_id, "action": "unmonitored" })); }
        }
    }
    if !report.errors.is_empty() { report.status = "error".into(); }
    store.upsert_artist(&ArtistCache { artist_id, fingerprint, processed_at: now_iso(), status: report.status.clone() }).await?;
    Ok(report)
}

fn build_full_track_index(full_albums: &[Value], tracks_by_album: &HashMap<i64, Vec<Value>>) -> HashMap<String, Vec<i64>> {
    ...
    for album in full_albums {
        if let Some(album_id) = id(album) {
            for track in tracks_by_album.get(&album_id).into_iter().flatten() {
                index.entry(normalize(&track_title(track))).or_default().push(duration_ms(track).unwrap_or_default());
            }
        }
    }
    index
...
fn classify_release(tracks: &[Value], full_track_index: &HashMap<String, Vec<i64>>, tolerance_ms: i64) -> &'static str {
    if tracks.is_empty() { return "REVIEW"; }
    let mut matched = 0;
    ...
    for track in tracks {
        ...
        match full_track_index.get(&key) {
            Some(durations) if durations.iter().any(|d| (*d - duration).abs() <= tolerance_ms) => matched += 1,
            Some(_) => needs_review += 1,
            None => return "KEEP",
        }
    }
    if matched == tracks.len() { "DUPLICATE" } else if needs_review > 0 { "REVIEW" } else { "KEEP" }
}

fn normalize(value: &str) -> String {
    value.to_lowercase().chars().map(|c| if c.is_ascii_alphanumeric() { c } else { ' ' }).collect::<String>().split_whitespace().collect::<Vec<_>>().join(" ")
}

fn artist_fingerprint(artist: &Value, cfg: &AlbumConfig) -> String {
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    format!("{}:{}:{}:{}", id(artist).unwrap_or_default(), artist_name(artist), bool_field(artist, "monitored").unwrap_or(false), cfg.target_metadata_profile_id).hash(&mut hasher);
    format!("{:x}", hasher.finish())
}

fn cache_hit(row: &ArtistCache, fingerprint: &str, max_age_days: i64) -> bool {
    if row.status != "success" || row.fingerprint != fingerprint { return false; }
    DateTime::parse_from_rfc3339(&row.processed_at)
        .map(|dt| Utc::now().signed_duration_since(dt.with_timezone(&Utc)).num_days() <= max_age_days)
        .unwrap_or(false)
}
